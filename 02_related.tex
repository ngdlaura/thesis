...
% The component-level formulation can reduce the input sequence length for a document, hence decreasing the computational complexity at training and inference time 
...
%To model the inherent hierarchical formulation of a document layout, \ac{LAMPreT} uses two cascaded Transformers. The lower-level Transformer encodes each contents block (e.g. text, table, image), while the higher-level Transformer aggregates the block-level representations and connections obtained. To train the higher-level model, the authors design structure-exploiting pre-training objectives: (1) block-order predictions, (2) masked block predictions, and (3) image fitting predictions. 
