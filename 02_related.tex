...
\begin{equation}
\begin{aligned}
    \text{ROUGE-}n_{\text{precision}} &= \dfrac{\vert \text{ predicted } n\text{-grams } \cap \text{ reference } n\text{-grams } \vert}{\vert \text{\text{ predicted } n\text{-grams }} \vert} \\
    \text{ROUGE-}n_{\text{recall}} &= \dfrac{\vert \text{ predicted } n\text{-grams } \cap \text{ reference } n\text{-grams } \vert}{\vert \text{\text{ reference } n\text{-grams }} \vert} \\
...
\begin{equation}
    \textrm{SA}^{(causal)}\bigl(\bm{y}^{(l)}_1, \ldots, \bm{y}^{(l)}_i\bigr) =
    {\begin{pmatrix}
...
\begin{equation}
    A(\bm{q}_i, \bm{K}, \bm{V}) = \sum_{j=1}^n \mathrm{Softmax}\left(\frac{{\bm{q}_i}^{\top} (\bm{k}_j + \bm{r}^K_{ij})^{\top}}{\sqrt{d_k}}\right) \cdot \left(\bm{v}_j + \bm{r}^V_{ij}\right),
\end{equation}

\noindent $\bm{r}^K_{ij}$ denotes the between tokens $i$ and $j$, and is expressed as follows:
