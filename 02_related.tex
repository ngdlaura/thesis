...
% Key tasks include \ac{NER}, which seeks to identify and classify named entities into pre-defined categories, and Relationship Extraction, where the goal is to determine relationships or connections between different entities mentioned in the text. 
...
% SentEval \citep{conneau2018senteval} is a toolkit crafted for evaluating the quality of universal sentence representations. It covers an array of tasks, including binary and multi-class classification, \ac{NLI}, and sentence similarity.
...
To advance research aimed at developing models with task-agnostic knowledge representations, the KILT benchmark \citep{petroni-etal-2021-kilt} was introduced. Comprising eleven datasets across five different tasks—including fact checking, open-domain question answering, slot filling, entity linking, and dialogue—the KILT benchmark is designed to evaluate models that must condition on specific information within large textual resources.
...
% Given $k \in \{1, \ldots, n-1\}$, Equation~\ref{equation:causal-distribution} can be generalized as follows:

% \noindent For any $k \in \{1, \ldots, n-1\}$, the conditional distribution of $w_{k+1}, \ldots, w_n$ given $w_1, \ldots, w_k$ can be expressed as follows:
...
% the multiplicative inverse of $P(w_1, \ldots, w_n)$, normalized by the number of words in the test set:
...
% quantifying the efficiency of encoding a sequence of tokens using a specific model
...
%Similar to perplexity, cross-entropy is minimized during training.
...
% \noindent The distribution of probabilities over the output is computed as:
...
% Recurrent connections in \acp{RNN} allow information to be retained and updated over time, enabling them to capture sequential dependencies. 
% Training \acp{RNN} involves minimizing cross-entropy using the \ac{BPTT} algorithm \citep{werbos1990backpropagation}. \ac{BPTT} unrolls the computational graph one time step at a time, creating a feed-forward network with repeated parameters. Gradients are backpropagated through the unrolled network and accumulated to update the parameters $\bm{\theta}$.
...
% To address these challenges, variants of \acp{RNN} have been introduced, with \ac{LSTM} being a prominent solution \citep{hochreiter1997long}. \ac{LSTM} introduces gated structures that enable selective retention or forgetting of information over time. Unlike vanilla \acp{RNN}, \acp{LSTM} consist of more complex modules, called cells, equipped with gates. These gates, driven by sigmoid activation functions, regulate the flow of information by determining what to forget, store, and output. The gating mechanism in \acp{LSTM} empowers them to control information flow, mitigating the vanishing gradient problem and enhancing their ability to handle long sequences while preserving long-term dependencies.
...
% \ac{RNN}-based language models process the input sequence one at a time, predicting the next token based on the current token and the previous hidden state. 
% \acp{RNN} theoretically have an unrestricted context, with the hidden state ideally encapsulating information about all prior tokens in the sequence. Demonstrating superior performance over $n$-gram models, \ac{RNN}-based language models produce more natural-sounding text in various setups \citep{mikolov2010recurrent, kovavcevic2022bidirectional}. After training, the final state $\bm{h}_{n}$ represents the entire text $(w_1, \ldots, w_n)$, and can be used in diverse classification and regression tasks, such as speech recognition \citep{schwenk2007continuous} and morpho-syntactical labeling tasks \citep{collobert2011deep}.
% }
...
% Let $(\bm{x}_1, \ldots, \bm{x}_n) \in \mathbb{R}^d$ be the input sequence of a convolutional layer, $\bm{K} \in \mathbb{R}^{w \times d' \times d}$ a kernel of width $w$, and $b \in \mathbb{R}^{d'}$ the bias term. One-dimensional convolution can be defined as:
...
% In the first layers of a \ac{CNN}, convolution is applied to token representations, \textit{i.e.}, $(\bm{x}_1, \ldots, \bm{x}_n)$ corresponds to a sequence of distributed representation of tokens. 

In \ac{NLP}, \acp{CNN} are commonly used for static classification tasks, including sentiment analysis \citep{kalchbrenner2014convolutional}, relation extraction \citep{nguyen2015relation}, and entity recognition \citep{adel2016comparing}. \acp{CNN} can integrate information from large context windows, learn high-level abstraction patterns, and exhibit greater stability compared to \acp{RNN} in handling the vanishing gradient problem. However, \acp{CNN} face more limitations when applied to text in contrast to images. For tasks like language modeling, deeper models tend to harm performance \citep{pham2016convolutional}. This is attributed to the distinct nature of visual and linguistic data, with convolution in language processing potentially distorting the input to the point where it is no longer recognizable as text.
